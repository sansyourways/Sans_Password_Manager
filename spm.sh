...
domain_dns_report() {
	...
	if [ -z "$resolved" ]; then
		printf 'DNS      : %s has no A record yet. Create it before continuing,\n' "$domain"
		printf '           or the certificate request will fail.\n'
		return 0
	fi
	printf 'DNS      : %s -> %s\n' "$domain" "$resolved"
	if [ "$proxied" = "1" ]; then
...
domain_setup_flow() {
	local port="$1" domain names proxied answer email body prior
	SPM_DOMAIN_READY=""
	...
	printf '\n'
	for answer in $names; do domain_dns_report "$answer" "$proxied"; done
	...
	prior=""
	if [ "${SPM_ACME_DRY_RUN:-0}" = "1" ] &&
		sudo test -f "/etc/nginx/sites-available/$domain"; then
	...
	if [ "${SPM_ACME_DRY_RUN:-0}" = "1" ]; then
		...
		printf 'to finish.\n'
		if [ -n "$prior" ]; then
			domain_install_vhost "$domain" "$(cat "$prior")" >/dev/null &&
				printf 'The previous vhost for %s was restored.\n' "$domain"
			rm -f "$prior"
		fi
		return 0
	fi
